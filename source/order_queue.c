...
int queue_complete_orders_floor(queue_node **head, int floor, HardwareMovement g_previous_direction){
    ...
    while (current != NULL){
        if (current->floor == floor){
            if (!queue_check_if_ignore(&current, floor, g_previous_direction, lowest_ordered_floor, highest_ordered_floor)){
                return 1;
            }
        }
    current = current->next;
...
int queue_check_if_ignore(queue_node ** head, int g_current_floor, HardwareMovement g_previous_direction, int lowest_ordered_floor, int highest_ordered_floor){
    ...
    if ((*head)->floor == g_current_floor){

        if (g_previous_direction == HARDWARE_MOVEMENT_UP){
        ...
        else if (g_previous_direction == HARDWARE_MOVEMENT_DOWN && (*head)->floor != 0){
